...
class SerialDemoScreen extends StatefulWidget {
  const SerialDemoScreen({super.key});

  @override
  _SerialDemoScreenState createState() => _SerialDemoScreenState();
}

class _SerialDemoScreenState extends State<SerialDemoScreen> {
  UsbPort? _port;
  StreamSubscription<Uint8List>? _subscription;
  List<UsbDevice> _devices = [];
  UsbDevice? _selectedDevice;
  final TextEditingController _sysTraceController =
      TextEditingController(text: 'hsldks3478257');
  final TextEditingController _sysSNController =
      TextEditingController(text: 'hsl864s3478257');
  final TextEditingController _transTypeController =
      TextEditingController(text: '3');
  final TextEditingController _transAmountController =
        ...
        if (_port != null) {
          ...
          setState(() {
            _portController.text =
                'Port is null, device not connected properly.';
          });
        }
      } catch (e) {
        setState(() {
        ...
        if (startIdx != -1 && endIdx != -1 && endIdx > startIdx) {
          ...
          if (buffer.length >= startIdx + 3 + dataLength + 1) {
            final completeMessage =
                buffer.sublist(startIdx + 3, startIdx + 3 + dataLength);

            // Convert the message to a hex string (for debugging)
            final hexString = completeMessage
